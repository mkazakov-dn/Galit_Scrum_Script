...
def collect_device_data(serial):
    ...
    try:
        # Create a unique log file name for each device
        log_file = os.path.join(os.getcwd(), f'{serial}_ssh_log')
        ssh = SSH_Conn(host=serial, icmp_test=False, session_log=log_file)
        ...
        fabric_transceiver_output = ssh.exec_command("show interfaces fabric transceiver")
        backplane_output = ssh.exec_command("show system backplane")
        power_output = ssh.exec_command("show system hardware power")
        
        # Parse transceiver output into interface blocks
        interface_blocks = parse_transceiver_output(transceiver_output)
        fabric_interface_blocks = parse_transceiver_output(fabric_transceiver_output)
        ...
        device_data = {
            ...
            "fabric_interfaces": fabric_interface_blocks,
            "backplane_output": backplane_output,
